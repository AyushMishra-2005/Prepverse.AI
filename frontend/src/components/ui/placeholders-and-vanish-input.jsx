...
import { cn } from "../../lib/utils.js"; // Make sure cn() is defined or remove
...
export function PlaceholdersAndVanishInput({ placeholders, onChange, onSubmit }) {
  ...
  const vanishAndSubmit = () => {
    ...
    if (val) {
      const maxX = newDataRef.current.reduce((max, cur) => (cur.x > max ? cur.x : max), 0);
      animate(maxX);
    }
  };
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className={cn(
          "w-full relative text-sm sm:text-base z-50 text-white bg-transparent h-full rounded-full focus:outline-none focus:ring-0 pl-4 sm:pl-10 pr-20 placeholder:text-zinc-400",
        ...
        <AnimatePresence mode="wait">
          {!value && (
            <motion.p
              ...
              exit={{ y: -15, opacity: 0 }}
              transition={{ duration: 0.3, ease: "linear" }}
              className="text-zinc-400 text-sm sm:text-base font-normal pl-4 sm:pl-12 w-[calc(100%-2rem)] truncate"
            >
